...
namespace lslidar_driver
{
	...
	int LslidarDriver::getScan(std::vector<ScanPoint> &points, ros::Time &scan_time, double &scan_duration)
	{
	...
	bool LslidarDriver::initialize()
	{
		...
		{
			unsigned char start_cmd[7] = {0xA5, 0x5A, 0x55, 0x01, 0x01, 0xFA, 0xFB};
			for (int i = 0; i < 5; ++i)
			{
				int w = serial_->send((const char*)start_cmd, 7);
				if (w == 7)
				{
					ROS_INFO("N10 start scan command sent");
					break;
				}
				usleep(200000);
			}
	...
	int LslidarDriver::receive_data(unsigned char *packet_bytes)
	{
		...
		ROS_INFO_THROTTLE(10.0,"Received data length is: %d", len);
		return len;
	}
	...
	void LslidarDriver::difop_processing(unsigned char *packet_bytes)
	{
		...
		ROS_INFO_THROTTLE(10.0,"degree_compensation = %f\n", degree_compensation);
		return;
	}
	...
	void LslidarDriver::data_processing(unsigned char *packet_bytes, int len)
	{
		...
		for (int num = 0; num < package_points; num++)
		{
			...
			bool crossed_zero = (scan_points_[idx].degree < last_degree &&
								scan_points_[idx].degree < 10 &&
								last_degree > 350);
			bool buffer_full = (idx >= points_size_ - 10);

			if (crossed_zero || buffer_full)
			{
				...
				int scan_points_size = scan_points_.size();
				...
				msg.ranges.resize(count_num);
				msg.intensities.resize(count_num);
				
				for (int i = 0; i < count_num; ++i)
				{
					float angle_deg = scan_points_[i].degree;
					int idx = int(round((360.0 - angle_deg) * count_num / 360.0)) % count_num;
					msg.ranges[idx] = scan_points_[i].range;
					msg.intensities[idx] = scan_points_[i].intensity;
				}
			...
			else
			{
				last_degree = scan_points_[idx].degree;
				idx++;
				// ROS_INFO("ERROR WITH BUFFER AND CROSSING ZERO IN data_processing...");
			}
		}
		// ROS_INFO("data_processing....");
	}
